...
static DELAY_MS: AtomicU32 = AtomicU32::new(250);
static RX_PIPE: StaticCell<Pipe<NoopRawMutex, 512>> = StaticCell::new();

#[unsafe(link_section = ".bi_entries")]
#[used]
pub static PICOTOOL_ENTRIES: [embassy_rp::binary_info::EntryAddr; 4] = [
    embassy_rp::binary_info::rp_program_name!(c"Pico2W USB Delay Console"),
    embassy_rp::binary_info::rp_program_description!(c"Set LED delay via USB CDC"),
...
async fn usb_console_task(
    ...
    pipe: &'static Pipe<NoopRawMutex, 512>,
) -> ! {
    ...
    loop {
        ...
        match select(tick, read_fut).await {
            Either::First(()) => {
                let d = DELAY_MS.load(Ordering::Relaxed);
...
#[embassy_executor::task]
async fn usb_rx_pump(
    mut rx: embassy_usb::class::cdc_acm::Receiver<'static, rp_usb::Driver<'static, USB>>,
    pipe: &'static Pipe<NoopRawMutex, 512>,
) -> ! {
    let mut pkt = [0u8; 64];
    loop {
        match rx.read_packet(&mut pkt).await {
            Ok(n) if n > 0 => {
                let _ = pipe.write(&pkt[..n]).await;
            }
            _ => {}
        }
    }
...
fn parse_u32_ascii(s: &str) -> Result<u32, ()> {
    let mut v: u32 = 0;
...
async fn main(spawner: Spawner) {
    ...
    config.product = Some("USB CDC Delay Console");
    ...
    let pipe = RX_PIPE.init(Pipe::new());

    spawner.spawn(usb_dev_task(usb)).unwrap();
    spawner.spawn(usb_rx_pump(rx, pipe)).unwrap();
    spawner.spawn(usb_console_task(tx, pipe)).unwrap();
    ...
    loop {
        let d = DELAY_MS.load(Ordering::Relaxed) as u64;
        let delay = Duration::from_millis(d.max(1));
